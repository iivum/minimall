...
import com.minimall.dto.OrderResponseDTO;
...
public class OrderController {
    ...
    public ResponseEntity<List<OrderResponseDTO>> getUserOrders(@PathVariable String userId) {
        ...
        return ResponseEntity.ok(orderService.findByUserId(userId).stream().map(OrderResponseDTO::from).toList());
    }
    ...
    public ResponseEntity<OrderResponseDTO> getOrderByNo(@PathVariable String orderNo) {
    ...
    public ResponseEntity<OrderResponseDTO> createOrder(@Valid @RequestBody CreateOrderRequest request) {
        ...
        return ResponseEntity.ok(OrderResponseDTO.from(orderService.create(request.userId, request.items)));
    }

    @PatchMapping("/{id}/status")
    @Operation(summary = "Update order status")
    public ResponseEntity<OrderResponseDTO> updateStatus(@PathVariable String id, @RequestParam Order.Status status) {
        Order order = orderService.findById(id);
        if (!securityUtils.isCurrentUser(order.getUser().getId())) {
            throw new UnauthorizedException("You can only update your own orders");
        }
        return ResponseEntity.ok(OrderResponseDTO.from(orderService.updateStatus(id, status)));
    }

    @PatchMapping("/{id}/pay")
    @Operation(summary = "Mark order as paid")
    public ResponseEntity<OrderResponseDTO> payOrder(@PathVariable String id, @RequestParam String tradeNo) {
        Order order = orderService.findById(id);
        if (!securityUtils.isCurrentUser(order.getUser().getId())) {
            throw new UnauthorizedException("You can only pay your own orders");
        }
        return ResponseEntity.ok(OrderResponseDTO.from(orderService.pay(id, tradeNo)));
    }
