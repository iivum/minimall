...
2. 检查 `src/test/java` 目录是否存在
3. 使用 `test -f` 验证具体测试文件存在（而非目录）
4. 执行 `verify-deliverables.sh pom.xml` 验证 pom.xml
5. 验证构建产物 JAR 文件存在
6. 生成验证报告
7. 上传验证报告作为 artifact

**verify-deliverables.sh 已支持文件列表参数:**

```bash
./scripts/verify-deliverables.sh pom.xml src/main/java/.../X.java
```
...
**Agent 在标记 `in_review` 前必须:**

1. 执行 `verify-deliverables.sh` 验证关键文件存在
2. 在 issue comment 中提供 `test -f` 验证结果的客观证据
...
| 1.1 | 2026-05-18 | 更新 CI 验证具体文件而非目录，增加 Pre-review 验证要求 (MIN-2666) |