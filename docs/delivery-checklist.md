...
```bash
# 验证 PR 已合并到 main 并截图证明
gh pr list --state merged --base main --limit 100 | grep <PR编号>
```

**通过标准**: PR 出现在 merged 列表中，并附带截图证明

> ⚠️ **所有 PR 必须附带 main 分支合并证明截图**
>
> 截图要求：
> 1. 显示 PR 状态为 "Merged"
> 2. 显示目标分支为 `main`
> 3. 显示 PR 编号和标题
> 4. 在 issue 下发布截图作为交付证明
